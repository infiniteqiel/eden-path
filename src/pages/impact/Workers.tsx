...
import { Users, Heart, DollarSign, GraduationCap, CheckSquare2, MessageSquare, Home, Plus } from 'lucide-react';
...
const Workers = () => {
  ...
  useEffect(() => {
    if (currentBusiness) {
      ...
      loadSubAreasByImpact(currentBusiness.id, 'Workers');
    }
  ...
  const handleCreateSubArea = async (title: string, description?: string) => {
    ...
    try {
      await createSubArea(currentBusiness.id, 'Workers', title, description);
  ...
  const getTasksForSubArea = (subAreaId: string) => {
    return workersTodos.filter(todo => todo.subAreaId === subAreaId);
  };

  // Get unassigned tasks for the "All Tasks" section
  const unassignedTasks = workersTodos.filter(todo => !todo.subAreaId);

  // Get the dragged task for overlay
  const activeTask = activeId ? workersTodos.find(t => t.id === activeId) : null;
              ...
              <div className="space-y-8">
                ...
                <section className="bg-white/80 backdrop-blur-sm rounded-xl p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold">Worker Impact Areas</h3>
                    <Button 
                      variant="outline" 
                      size="sm"
                      ...
                      {subAreas.map((subArea) => (
                        <DroppableSubArea
                          ...
                          icon={subArea.isUserCreated ? Plus : 
                                subArea.title.toLowerCase().includes('compensation') ? DollarSign :
                                subArea.title.toLowerCase().includes('well') ? Heart :
                                subArea.title.toLowerCase().includes('career') ? GraduationCap : Users}
                          impactArea="Workers"
                ...
                <section className="bg-white/80 backdrop-blur-sm rounded-xl p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold">All Workers Tasks</h3>
                  ...
                  {unassignedTasks.length > 0 ? (
                    ...
                    <div className="text-center py-12 animate-fade-in">
                      ...
                      <p className="text-muted-foreground">No unassigned workers tasks</p>
