...
import { MessageSquare, Send, Bot, User, History, ChevronRight } from 'lucide-react';
...
export function EnhancedAIChatModal({ 
  ...
  const getContextLabel = (session: ChatSession) => {
    if (session.level === 'task' && session.task_id) {
      return `Task Chat`;
    } else if (session.level === 'subarea' && session.specific_area) {
      return `${session.specific_area} Specialist`;
    } else if (session.level === 'overview' && session.impact_area) {
      return `${session.impact_area} Overview`;
    }
    return 'General Chat';
  };
          ...
          {showHistory && (
            <div className="w-64 border-r border-gray-200 bg-gray-50 flex flex-col">
              <div className="p-3 border-b border-gray-200">
                <h3 className="font-medium text-sm text-gray-700">Chat History</h3>
              </div>
                ...
                <div className="p-2 space-y-1">
                  {chatSessions.map((session) => (
                      <Button
                        ...
                        onClick={() => {
                          // This would switch to that session's context
                          console.log('Switch to session:', session.id);
                        }}
                      >
                        <div className="flex flex-col items-start w-full">
                          <span className="text-xs font-medium">{getContextLabel(session)}</span>
