...
CREATE TABLE public.user_document_categories (
...
CREATE INDEX idx_user_document_categories_business_id ON public.user_document_categories(business_id);
CREATE INDEX idx_user_document_categories_sort_order ON public.user_document_categories(business_id, sort_order);
...
CREATE INDEX idx_file_audit_log_business_id ON public.file_audit_log(business_id);
CREATE INDEX idx_file_audit_log_created_at ON public.file_audit_log(created_at DESC);

-- Add category_id column to files table for user-defined categories
ALTER TABLE public.files ADD COLUMN category_id UUID REFERENCES public.user_document_categories(id) ON DELETE SET NULL;
...
-- Create trigger for task file mapping audit
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_files_dataroom_impact_area ON public.files(dataroom_id, impact_area) WHERE is_deleted = false;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_todos_business_status ON public.todos(business_id, status) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_task_file_mappings_file_task ON public.task_file_mappings(file_id, task_id);
